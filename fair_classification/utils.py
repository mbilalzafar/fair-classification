...
def test_sensitive_attr_constraint_cov(model, x_arr, y_arr_dist_boundary, x_control, thresh, verbose):
    ...
    # cov = np.cov(arr, x_control) # this gives the covariance matrix
    # assert(cov[0][1] == cov[1][0]) # these diagonal values are the covariance values, others diagonal values are variance
    # cov = cov[1][0]
